...
export const NIGERIA_STATES: { name: string; lgas: string[] }[] = [
  {
    name: "Abia",
    lgas: [
...
export function lgasForState(state: string): string[] {
  ...
  return NIGERIA_STATES.find((row) => row.name === state)?.lgas ?? [];
}

export function isNigeriaPlace(value: string | null | undefined) {
